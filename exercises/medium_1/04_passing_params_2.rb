...
dogs = %w(pug poodle great_dane pitbull)
categorize(dogs) do |pug, poodle, *big_dogs| # ignore the first two, should have put `_`!
  p big_dogs # => '['great dane', 'pitbull']'
end

languages = %w(English Korean Spanish French Italian)
categorize(languages) do |english, korean, *romantic|
  p romantic # => '['Spanish', 'French', 'Italian']'
end